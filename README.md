...
```mermaid
graph TD
    Client([👤 Internet Client]) -->|HTTP :80| Nginx[🌐 Nginx Load Balancer<br/>frontend_net]
    
    subgraph "Application Tier (Replicated)"
        Nginx -->|Round Robin| API1[⚙️ Node.js API - Replica 1]
        Nginx -->|Round Robin| API2[⚙️ Node.js API - Replica 2]
        Nginx -->|Round Robin| API3[⚙️ Node.js API - Replica 3]
    end

    subgraph "Data Tier (Isolated)"
        API1 -->|TCP :6379| Redis[(🗄️ Redis Database<br/>backend_net)]
        API2 -->|TCP :6379| Redis
        API3 -->|TCP :6379| Redis
    end

    Redis -.- Vol[💾 Docker Volume: cache_data]

    classDef pub fill:#e1f5fe,stroke:#4a90e2,stroke-width:2px;
    classDef priv fill:#ffebee,stroke:#d32f2f,stroke-width:2px;
    
    class Nginx pub;
    class API1,API2,API3 pub;
    class Redis priv;
```
